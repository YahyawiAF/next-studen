...
import { deleteClass, fetcher } from '../../utils/apiClient';
...
function ClassList({ setPage, setRow }: any) {
    ...
    const { data, mutate } = useSWR(keyword ? `/api/class?page=${vpage - 1}&keyword=${keyword}` : `/api/class?page=${vpage - 1}`, fetcher)
        ...
        <>
            ...
            <div className="py-4 px-4 w-full md:w-full m-5 md:mx-10 flex flex-col space-y-6 items-center bg-white border rounded-lg drop-shadow-sm shadow-gray-100">
                ...
                <div className="w-full">
                    ...
                    <tr className={`hidden md:grid md:grid-cols-6 bg-[#f9f5f1] font-bold text-[#228e9d] text-sm tracking-wide rounded-lg mb-4 px-2 py-1`}>
                        <td className="p-1 px-2">Class ID</td>
                        <td className="p-1 px-2">Class Name</td>
                        <td className="p-1 px-2">Start date</td>
                        <td className="p-1 px-2">End date</td>
                        <td className="p-1 px-2">Teacher in charge</td>
                        <td className="p-1 px-2">Academic year</td>
                    </tr>
                            ...
                            <tr key={i} className={`grid md:grid-cols-8 text-gray-900 text-[0.81rem] font-normal font-circular rounded-md px-2 py-1 divide-y-3 border-b-2 border-slate-50`}>
                                <>
                                    <td className="md:p-1 md:px-2">
                                        <span className="px-4 py-1 w-full flex md:hidden rounded-md bg-[#f8f9fa] text-gray-500/70">Class ID</span>
                                        <div className="px-2 py-1 indent-4 md:indent-0">{row.refno}</div>
                                    </td>
                                    <td className="md:p-1 md:px-2">
                                        <span className="px-4 py-1 w-full flex md:hidden rounded-md bg-[#f8f9fa] text-gray-500/70">Class Name</span>
