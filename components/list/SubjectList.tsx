...
import { deleteSubject, fetcher } from '../../utils/apiClient';
...
function SubjectList({ setPage, setRow }: any) {
   ...
   const { data, mutate } = useSWR(keyword ? `/api/subject?page=${vpage - 1}&keyword=${keyword}` : `/api/subject?page=${vpage - 1}`, fetcher)
      ...
      <>
         ...
         <div className="py-4 px-4 w-full md:w-full m-5 md:mx-10 flex flex-col space-y-6 items-center bg-white border rounded-lg drop-shadow-sm shadow-gray-100">
            <div className="z-10 w-full sticky top-20  md:relative md:top-0 flex flex-col md:flex-row items-center justify-between bg-white md:bg-blue-50/10 border border-gray-300/70 focus:border-gray-600/70 rounded-md overflow-hidden">
               <span className="my-2 md:my-0 px-6 sm:text-lg tracking-widest font-bold text-[#000131] font-arial">Subjects</span>
            ...
            <div className="w-full">
               ...
               <tr className={`hidden md:grid md:grid-cols-6 bg-[#f9f5f1] font-bold text-[#228e9d] text-sm tracking-wide rounded-lg mb-4 px-2 py-1`}>
                  <td className="p-1 px-2">Subject ID</td>
                  <td className="p-1 px-2">Subject Name</td>
                     ...
                     <tr key={i} className={`grid md:grid-cols-8 text-gray-900 text-[0.81rem] font-normal font-circular rounded-md px-2 py-1 divide-y-3 border-b-2 border-slate-50`}>
                        <>
                           <td className="md:p-1 md:px-2">
                              <span className="px-4 py-1 w-full flex md:hidden rounded-md bg-[#f8f9fa] text-gray-500/70">Subject ID</span>
                              <div className="px-2 py-1 indent-4 md:indent-0">{row.refno}</div>
                           </td>
                           <td className="md:p-1 md:px-2">
                              <span className="px-4 py-1 w-full flex md:hidden rounded-md bg-[#f8f9fa] text-gray-500/70">Subject Name</span>
